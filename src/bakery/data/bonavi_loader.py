...
_CATEGORY_RULES: list[tuple[str, list[str]]] = [
    # salad/deli — 당일폐기 품목이지만 "구운 빵"이 아님. 예측 타깃에서 제외(사용자 결정 2026-07-23).
    # 반드시 다른 규칙보다 먼저 평가 (샐러드류가 빵 키워드에 오분류되지 않도록).
    ("salad", [
        "샐러드", "보울",
    ]),
    ("cake", [
    ...
    ("sandwich", [
        ...
        "샌드위치",  # plain 샌드위치 (에그마요/크림치즈/치킨 등) — 옛 분류기 누락분
    ]),
    ("pastry", [
        # 페이스트리
    ...
    ("bread", [
        ...
        "호밀", "통밀", "잡곡", "곡물브레드", "곡물 브레드", "그레인롤", "그레인 롤",
        "크림치즈브레드", "크림치즈 브레드",
        "베리 브레드", "크림브레드", "크림 브레드",
        "탕종", "맘모스",
        "마늘", "어니언", "부추", "토마토",

...
<div class="desc" style="margin-top:12px;"><b>발주 영향</b>(광교 홀드아웃, prior-off vs on 통제 A/B) — 버퍼(N·K)는 <b>안 바뀝니다</b>: 버퍼는 12개월 aggregate 최적점이라
 크리스마스 1~2일이 그 최적을 못 움직이고, 게다가 버퍼 최적화 창(calib 앞 6개월)엔 이벤트가 아예 없습니다(추석·크리스마스는 전부 뒤 6개월).
 대신 <b>예측 자체가 고쳐져 같은 버퍼로 이벤트일이 커버</b>됩니다 — 매진율 daily 0.071→0.060 · weekly 0.055→0.044, <b>총부족 절반</b>(daily 155→80, weekly 127→55), <b>폐기 거의 불변</b>(+1%). 즉 "버퍼 축소"가 아니라 "같은 발주로 매진↓"이 실제 효과. (아래 발주 정책 차트는 prior 반영 qmat 기준.)</div>

...
- `auto_approve` — 즉시 APPROVED (autonomous, frontier 연출). 게이트 단일 레버 원칙상 closed-loop의 store는 항상 `require_approval=True`이고, autonomous는 이 정책으로 표현한다(`require_approval=False` 경로는 미사용).
...
| CLI `closed-loop` | `cli` | end-to-end 실행, 제안·게이트 결정·확정시트 출력, `--out` parquet 저장, `--policy auto\|human` 게이트 선택 |

## 데이터 흐름 (예시)

```
$ uv run bakery closed-loop --store 광교 --period 2026-07-06,2026-07-12 --policy human

...
## 11. 3단계 결과 — KPI를 harness 지표로 편입 (완료)

`bakery harness-run experiments/gwangyo_kpi.yaml` **한 줄**로 비용·매진·아띠제 절감률이 나온다.
단일 출처 = **`src/bakery/evaluation/order_kpi.py`**(신설). 재구현 0 — `order_cost` /
`category_stockout_cost` / `simulate_item_day_kpis` / `build_arrival_profile` 호출만.

### 정의 5축 — 코드 한 곳에서 확정
| 축 | 확정 | 구현 |
|---|---|---|
| basis | **A/B 병기** | `basis_actual()` / `basis_sim()` |
| 원가율 | 품목별 0.40/0.60 | `cost_rate_for()` 재사용 |
| 품절 손실 | 전체매진만(k=0) | `category_stockout_cost()` |
| **SKU 품절율** | **날별 비율의 평균** | `_daily_mean_of_rates()` — 행 단위 평균 교정 |
| 매진시각 | **전체 median + 날별 median 평균 병기** | `_soldout_hour_stats()` |

**SKU 품절율 교정은 실제로 값을 바꾼다** — 같은 데이터에서 날별 평균 0.667 vs 행 단위 0.500
(날마다 품목 수가 달라서). 헌장 문구가 "각 날 품절 SKU 비율의 날별 평균"이므로 날별 평균이 정의다.
행 단위 계산은 `sku_soldout_rate_rowwise()` 로 남겨 회귀 비교만 가능하게 했다.

### ★52-fold 실측 (`gwangyo_kpi.yaml`, lead_days=5, 364일)

| 지표 | category_total | distributional_total | 아띠제 실측(A) |
|---|---|---|---|
| **폐기비용** | 32,851,409원 | **32,080,365원** | **44,469,400원** |
| 폐기량 | 14,806개 | 14,452개 | 19,257개 |
| **아띠제 대비 절감률** | **−26.13%** | **−27.86%** | — |
| 전체매진 부족량 | 1,986개 | 2,238개 | — |
| 전체매진 손실 | 5,004,609원 | 5,631,375원 | — |
| **총비용**(k=0) | 37,856,018원 | **37,711,740원** | — |
| 전체매진일 비율 | 0.313 | 0.321 | 0.011 |
| SKU 품절율(날별 평균) | 0.451 | 0.458 | 0.446 |
| 매진시각 median | 18.85시 | 18.81시 | — |
| 매진시각 날별median 평균 | 18.91시 | 18.86시 | — |
| 조기매진율(20시 이전) | 0.742 | 0.750 | — |

**distributional이 총비용에서 앞선다**(37.71M vs 37.86M, −0.38%) — PR#73의 카테고리 층위
결론(−0.38%)과 **부호·크기가 일치**한다. 폐기 −771,044원이 전체매진 손실 +626,766원을 상회.

### ⚠️★ −26~28%를 그대로 인용하면 안 된다 (미완 항목)

**모델(B)과 아띠제(A)는 잣대가 다르다** — A는 실측 폐기(`QT_OUT`), B는 `발주 − 실수요`다.
`−26.13%` 에는 **진짜 절감 + censoring(잣대) 효과가 섞여 있다.**

`compare_to_actual()` 에 동일-basis 슬롯을 만들어 뒀다:
| 키 | 의미 | 현재 |
|---|---|---|
| `vs_actual_pct` | 모델(B) vs 아띠제(A) — **잣대 다름** | ✅ 산출 |
| `vs_actual_sim_pct` | 모델(B) vs **아띠제를 B로 시뮬** — 공정 비교 | ❌ **4단계** |
| `censoring_gap_pct` | 잣대 효과 크기 | ❌ 4단계 |

→ **4단계에서 `actual_sim`(아띠제 실생산을 B로 재측정)을 배선해야** 옛 −37~45%와 축별로 비교된다.

### ⚠️ 전체매진일 비율의 A/B 격차가 크다 (0.011 vs 0.313)
A는 "그날 총폐기 ≤ 5"(실측 폐기가 거의 항상 5 초과 → 전체매진 거의 없음), B는 "Σ발주 < Σ실수요"다.
**같은 관점①의 두 잣대이며 값이 28배 차이 난다** — 이것이 censoring의 크기다. 한쪽만 인용하면
"우리가 아띠제보다 30배 더 매진시킨다"는 오독이 나온다. 표가 둘을 나란히 두는 이유다.

### 음수 폐기 — 매 실험 산출에 박았다
`actual_waste_clip_effect_pct = 5.54%` (494/13,359행, −1,010개). 아띠제 확인 전까지
절감률 분모가 이만큼 자의적임을 리포트가 스스로 말한다(절대 규칙 #6).
⚠️ 판매 반품(`SALES_FG=1`, 1.79%)은 **별개 소스이며 전처리 백본에서 이미 net-out** 된다
(`bonavi_loader.py:377`) — 음수 폐기와 혼동 금지.

### 안전장치
- **`kpi: true` + `order_level: category` → `SpecError`**(실행 전 차단)
- **기본 off** — 헤드라인 실험은 무거운 KPI 경로를 타지 않는다
- **A basis를 모델이 평가한 같은 날짜로 제한** — 안 하면 기간 차이가 절감으로 오독된다
- 매진시각 컬럼이 없으면 관련 키를 **만들지 않는다**(가짜 nan 금지)

## 12. 4단계 결과 — 정산 완료 (전 단계 종료)

`actual_sim`(아띠제 실생산을 **B 잣대로** 재측정) 배선 → 세 축이 분리된다.
상세 정산 = **`docs/kpi_plane_reconciliation.md`**.

### ★★옛 수치는 재현된다 — 축을 잘못 맞췄던 것이다
| 축 | 옛 헤드라인 | 새 백본(52-fold) |
|---|---|---|
| `ΔvsA`(하한) | −30.9 ~ −41.2% | −26.1 ~ −27.9% |
| **`ΔvsB`(공정)** | **−32.4 ~ −46.6%** | **−33.4 ~ −35.0%** |
| **censoring gap** | **+10.2%** | **+10.97%** |

**censoring gap이 +10.2% → +10.97%로 거의 같다.** 잣대 효과 크기가 독립 재현됐고, 이것이
두 파이프라인이 같은 것을 재고 있다는 가장 강한 증거다. 내가 3단계에서 낸 −26~28%는
`ΔvsA` 축이었고 옛 −37~45%는 `ΔvsB` 축이었다 — **다른 축이었을 뿐 불일치가 아니다.**

### 옛↔새 범위 차이의 축별 분해
옛 `ΔvsB` −32.4~−46.6% → 새 −33.4~−35.0%(범위 좁아짐). **하단 −46.6%가 사라진 주된 이유는
NK 정책 제거**(과적합 판정). 남은 차이는 fold 규약(8×8주→52×7일) / 리드타임(0→5일) /
배분 leakage 수정 / 원가율(0.30→품목별)이 섞인 것이며 **전부 새 값이 더 보수적**이다.
→ **수치가 낮아진 것은 성능 저하가 아니라 측정이 엄격해진 것이다.**

### 트레이드오프가 표에서 정확히 보인다
아띠제 실생산을 B로 재면 **전체매진 부족량 1.8개(0.3%일)** — 과발주하니 거의 안 매진된다.
우리는 1,986~2,238개(31~32%일). **폐기 −33~35%의 대가가 이것이다.**
총비용(k=0) 기준으로는 우리가 **−23.6%**(37.71M vs 49.35M, 동일 B 잣대) — 전체매진 손실을
다 물어도 폐기 절감이 압도한다.

### 스크립트 축소 + 라우팅 갱신
- `scripts/unified_policy_kpi.py` → **레거시·5정책 비교 전용**으로 헤더 명시. 헤드라인 아님을
  못 지나치게 박고, 백본과 다른 축 4개(NK 폐기 / 원가율 0.30 / fold 8×8주 / leakage 미수정)를 열거.
- `.claude/CLAUDE.md` "아직 백본 밖인 것" 절 → **발주 KPI 백본 절로 교체**. 인용할 축이
  `ΔvsB`임을 못 지나치게 명시.

### 성공 기준 대조 (§5)
| # | 기준 | 결과 |
|---|---|---|
| 1 | harness-run 하나로 전부 | ✅ `gwangyo_kpi.yaml` |
| 2 | 엔진 동등성 rtol=1e-9 무변경 | ✅ 4단계 모두 통과 |
| 3 | 배분 컷오프가 lead_days 준수 + 테스트 | ✅ PR#74 |
| 4 | A/B 병기 + 정의 5축 한 곳 | ✅ `order_kpi.py` |
| 5 | 전체 스위트 green | ✅ 919 → **952 passed** |
| 6 | `unified_policy_kpi.py` wrapper화 | ✅ 레거시 명시(5정책 축만 남김) |
| 7 | 옛 수치 재측정 + 축별 분해 | ✅ `docs/kpi_plane_reconciliation.md` |

### 남은 열린 항목
- **음수 폐기**(`QT_OUT<0`, 3.70%, clip 효과 5.54%) — 아띠제 확인 필요. 매 산출에 박혀 있다.
- **`panel` + `item`** 조합 미지원(fails-loud) — 패널 KPI는 별도 과제.
- **고객경험 비용**은 여전히 모델 밖 — 전체매진일 0.3% → 31~32%는 마진으로만 상쇄된 것이다.

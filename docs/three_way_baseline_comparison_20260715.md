...
**매진 지표 정정**: "풀매진=전 품목 *동시* 매진"은 ~44품목이라 사실상 0(무의미). 카테고리 매진의
실질 지표는 **총량소진(Σ수요>Σ발주 → 카테고리 동남) = 카테고리매진①**.

기준 actual_production: 폐기 **10.44M** · 카테고리매진①(총량소진) 0.000 · item매진② 0.022

| 정책 | 폐기(KRW) | vs 실생산 | 카테고리매진①(총량소진) | item매진② |
|---|---|---|---|---|
| actual_production (기준) | 10.44M | — | 0.000 | 0.022 |
| **our_cat_quantile** q0.85 | 6.21M | **−40.5%** | 0.406 | 0.497 |
| **our_cat_conformal** s0.85 | 7.00M | **−32.9%** | 0.312 | 0.460 |
| our_cat_nk (q0.85+40) | 11.46M | +9.8% | 0.058 | 0.272 |
| artisee_reimpl | 9.23M | −11.6% | 0.246 | 0.311 |

*(폐기 음수=덜 버림=좋음. 매진 낮을수록 좋음, [[project_kpi_priority_framing]] 상 2차. 전 품목 동시매진은 전 정책 0.000=무의미.)*

**해석(폐기 1차·매진 2차)**:
- **우리 총량 발주(quantile −40.5% / conformal −32.9%)가 실생산 대비 폐기 크게 절감** — 핵심 성과.
- **대가 = 카테고리 매진(총량소진) 크게 증가**: quantile 0.41 vs 실생산 ~0. 우리 모델이 카테고리를
  훨씬 자주 동나게 함(q0.85 총량 under-calibration). 흡수 가정상 매출영향 작으나 고객경험 방향이라
  **2차로 관리**(hard constraint 아님). item매진(②)도 0.50으로 증가.
